...
var profileLauncher = {
  ...
  //PS_firefox : "chrome://browser/content/browser.xhtml",
  PS_thunderbird : "chrome://messenger/content/messenger.xhtml",
  //PS_addressbook : "chrome://messenger/content/addressbook/addressbook.xhtml",
  prefs : Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefBranch),

  // Open About Profiles in a new tab
  openAboutProfiles() {
    //let tabmail = document.getElementById("tabmail");
    openContentTab("about:profiles");
    //tabmail.openTab("contentTab", {contentPage: "about:profiles",
  ...
  //getdefaultprofile : function() {
    //let defaultProfNum;
    //try {
    //defaultProfNum = ProfileService.defaultProfile;
    //} catch (e) {}
  //},

  //getprofileinuse : function() {
    //let currentProfName = ProfileService.currentProfile;
    //profileLauncher.setComplexPref("extensions.profileswitcher.profile.in_use",currentProfName);
  //},
  ...
  runStatusbarProfileButton : function(event,item) {
    if (event.which == 1) {
      if (event.target.nodeName == "toolbarbutton" || ((event.target.parentNode.id == "profileNameSBP" || event.target.id == "profileNameSBP") && event.button == 0)) {
        event.stopPropagation();
        var prof = profileLauncher.getComplexPref("extensions.profileswitcher.profile.button_launch");
  ...
  setProfileIcon : function() {
    var icon = "chrome://profilelauncher/content/skin/icons/button"+profileLauncher.prefs.getIntPref("extensions.profileswitcher.icon_color")+".png";
  ...
  migratePrefs : function() {
    if (profileLauncher.prefs.getPrefType("extensions.profileswitcher.onload_disable_noremote") > 0) {
      try {
        profileLauncher.prefs.clearUserPref("extensions.profileswitcher.onload_disable_noremote");
      }
      catch(e) {}
    }
    if (profileLauncher.prefs.getPrefType("profileswitcher.close_before_launch") > 0) {
      try {
        ...
        profileLauncher.prefs.clearUserPref("profileswitcher.close_before_launch");
        ...
        profileLauncher.prefs.clearUserPref("profileswitcher.where_show_name");
        ...
        profileLauncher.prefs.clearUserPref("profileswitcher.load_current_page");
        ...
        profileLauncher.prefs.clearUserPref("profileswitcher.use_onbeforeunload");
        ...
        profileLauncher.prefs.clearUserPref("profileswitcher.hide_menus");
        //profileLauncher.prefs.clearUserPref("profileswitcher.profile.in_use_number");
      }
      catch(e) {}
    }
  ...
  runExec: function(item) {
    ...
    if (quit) {
      ...
      if (profileLauncher.prefs.getBoolPref("extensions.profileswitcher.use_onbeforeunload") ) {
        ...
        process.run(false,args,args.length);
      setTimeout(function() {window.close()}, quitDelay);
    }
    else
      process.run(false,args,args.length);
  },
  ...
  restartWithLog : function() {
    var param = {};
    param.abort = true;
    openDialog("chrome://profilelauncher/content/logDialog.xhtml","", "chrome,modal,centerscreen", param);
    if (param.abort)
      return;
    var type = "";
    if (param.pop3)
  ...
  file2array : function() {
    ...
    istream.init(profileLauncher.profilesINI, 0x01, 0o444, 0);
  ...
  setLabel : function() {
    ...
    var text = "Profile Switcher\n"+profileLauncher.bundle.GetStringFromName("profileinuse")+": "+used_prof;
    try {
  ...
  refreshList : function() {
    profileLauncher.prefs.clearUserPref("extensions.profileswitcher.profile.in_use");
    profileLauncher.prefs.clearUserPref("extensions.profileswitcher.profiles.list");
  ...
  clean : function() {
    ...
    if (! wins.hasMoreElements()) {
      profileLauncher.prefs.clearUserPref("extensions.profileswitcher.profile.in_use");
      profileLauncher.prefs.clearUserPref("extensions.profileswitcher.profiles.list");
      // profileLauncher.prefs.clearUserPref("extensions.profileswitcher.profile.in_use_number");
      profileLauncher.prefs.clearUserPref("extensions.profileswitcher.moz_no_remote");
    }
  },

  openOptions : function() {
    open("chrome://profilelauncher/content/settings.xhtml","","chrome=yes,modal=yes,centerscreen=yes");
  },
  ...
  //setABLabel : function() {
    ...
    //}
    //var icon = "chrome://profilelauncher/content/skin/icons/user"+profileLauncher.prefs.getIntPref("extensions.profileswitcher.icon_color")+".png";
